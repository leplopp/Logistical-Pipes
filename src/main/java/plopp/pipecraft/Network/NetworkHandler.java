...
public class NetworkHandler {
	...
	public static void registerPayloadHandlers(RegisterPayloadHandlersEvent event) {
		...
		event.registrar(PipeCraftIndex.MODID)
				...
				.playToClient(ViaductLinkerListPacket.TYPE, ViaductLinkerListPacket.CODEC,
						ViaductLinkerListPacket::handle)
				...
				.playToServer(PacketUpdateSortedPositions.TYPE, PacketUpdateSortedPositions.CODEC,
						PacketUpdateSortedPositions::handle)
				...
				.playToServer(PacketUpdateTeleporterNames.TYPE, PacketUpdateTeleporterNames.CODEC,
						PacketUpdateTeleporterNames::handle)
				.playToServer(PacketUpdateTeleporterToggle.TYPE, PacketUpdateTeleporterToggle.CODEC,
						PacketUpdateTeleporterToggle::handle)
				...
				.playToClient(TravellingItemSyncPacket.TYPE, TravellingItemSyncPacket.CODEC,
						TravellingItemSyncPacket::handle)
				.playToClient(TravellingItemRemovePacket.TYPE, TravellingItemRemovePacket.CODEC,
						TravellingItemRemovePacket::handle);
	}
	...
	public static void sendTravelStateToAll(ServerPlayer sender, boolean resetModel) {
		TravelData data = ViaductTravel.activeTravels.get(sender.getUUID());
		int ticksPerChunk = data != null ? data.ticksPerChunk : 32;
		int defaultTicksPerChunk = data != null ? data.defaultTicksPerChunk : 32;

		TravelStatePacket packet = new TravelStatePacket(sender.getUUID(), ViaductTravel.isTravelActive(sender),
				ViaductTravel.getTravelYaw(sender.getUUID(), sender.level()),
				ViaductTravel.getTravelPitch(sender.getUUID(), sender.level()),
				ViaductTravel.getVerticalDirection(sender.getUUID(), sender.level()), resetModel, ticksPerChunk,
				defaultTicksPerChunk);
